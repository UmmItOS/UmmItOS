...
- **`MultiEffect`'s inverted mask draws nothing when its mask is empty or zero-sized**, not everything. For shapes that grow from nothing (the wake's circles), write a `ShaderEffect`: the source is `wake/wake.frag`, compiled next to it with `/usr/lib/qt6/bin/qsb --glsl "100 es,120,150" --hlsl 50 --msl 12 -o wake.frag.qsb wake.frag`. Commit both; the shell loads the `.qsb`, so installs need no shader tools.
