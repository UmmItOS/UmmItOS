...
- **Shared components** at the root are `Surface` (the material), `Flyout` (the bar dropdown used by Wi-Fi, Bluetooth and Volume), `FlyoutRow` and `FlyoutEmpty` (its list row and empty state), `Toggle`, `Slider`, `Spinner`, `MaterialIcon`, `Reveal` (the open/close animation) and `SoftCard` (a card whose fill fades out toward its edge, so no edge line shows). Reuse these rather than building one-off versions.
...
- **Hyprland's blur draws a line across a fading surface.** With `ignore_alpha`, blur switches on in one step where the surface's alpha passes the threshold, so a gradient wash gets a hard edge. A surface that fades out (the wallpaper picker) has its blur turned off by its own layer rule and dims itself.
